...
import { OrderStatus, MenuItem } from '@/lib/types';
import { 
  ChefHat, 
  TrendingUp, 
  Clock, 
...
export default function RestaurantSpace() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Header Resto Pro */}
      <div className="bg-white rounded-3xl p-6 border border-[#E2ECE5] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl brand-gradient text-white text-xs font-bold shadow-md hover:opacity-95 flex items-center gap-1.5"
          >
      ...
      {activeTab === 'orders' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              {preparingOrders.map((order) => (
                ...
                >
                  ...
                  <div className="pt-3 border-t border-[#E2ECE5] flex items-center justify-between">
                    <span className="text-xs text-gray-500">Destination : {order.deliveryAddress.neighborhood}</span>
                    <button
                      onClick={() => updateOrderStatus(order.id, 'ready_for_pickup')}
                      className="px-4 py-2 rounded-xl brand-gradient-orange text-white text-xs font-bold shadow-md hover:opacity-95 flex items-center gap-1.5"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      <span>Plat Prêt pour Coursier</span>
                    </button>
                  </div>
                </div>
              ))}
          ...
          {/* Col 3 : Prêtes / En Livraison (Ready & In Transit) */}
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 rounded-2xl bg-emerald-50 border border-emerald-200">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-extrabold text-xs text-emerald-900 uppercase tracking-wider">
                  3. Prises en charge / Livrées ({readyOrders.length + completedOrders.length})
                </h4>
              </div>
            </div>
      ...
      {activeTab === 'menu' && (
        <div className="bg-white rounded-3xl border border-[#E2ECE5] p-6 shadow-sm">
            ...
            <form onSubmit={handleCreateDish} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ex: Crevettes marinées, oignons caramélisés et riz cassé..."
                  className="w-full px-3 py-2 bg-[#F7FAF7] border border-[#E2ECE5] rounded-xl text-xs focus:bg-white focus:border-[#008235] focus:outline-hidden"
                />
              </div>
              ...
              <div className="pt-3 border-t border-[#E2ECE5] flex items-center justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl brand-gradient text-white text-xs font-bold shadow-md hover:opacity-95"
                >
                  Créer le plat
                </button>
              </div>
